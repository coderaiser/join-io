...
function join(options, req, res, next) {
    ...
    const isFunc = typeof options.minify === 'function';
    const isMinify = isFunc ? options.minify() : options.minify;
    const read = exec.with(readPipe, req, res);
    ...
    exec.if(!isMinify, function(error, namesObj) {
        const is = typeof namesObj === 'object';
